...
export default class RouteBoxer {
     ...
     *
     * @param {this.google.maps.LatLng[] | this.google.maps.Polyline} path The path along
     *           which to create boxes. The path object can be either an Array of
     *           this.google.maps.LatLng objects or a Maps API v2 or Maps API v3
     *           this.google.maps.Polyline object.
     * @param {Number} range The distance in kms around the route that the generated
     *           boxes must cover.
     * @return {this.google.maps.LatLngBounds[]} An array of boxes that covers the whole
     *           path.
     */
    async box(path, range) {
        ...
        if (path instanceof Array) {
            ...
            vertices = path;
        } else if (path instanceof this.google.maps.Polyline) {
            if (path.getPath) {
     ...
     */
    buildGrid_(vertices, range) {

        // Create a LatLngBounds object that contains the whole path
        var routeBounds = new this.google.maps.LatLngBounds();
        for (var i = 0; i < vertices.length; i++) {
     ...
     */
    getGridCoordsFromHint_(latlng, hintlatlng, hint) {
        var x, y;
     ...
     */
    getGridIntersects_(start, end, startXY, endXY) {
     ...
     *                    the grid line
     */
    getGridIntersect_(start, brng, gridLineLat) {
     ...
     */
    fillInGridSquares_(startx, endx, y) {
        var x;
        if (startx < endx) {
    ...
    getCellBounds_(cell) {
        return new this.google.maps.LatLngBounds(
            new this.google.maps.LatLng(this.latGrid_[cell[1]], this.lngGrid_[cell[0]]),
            new this.google.maps.LatLng(this.latGrid_[cell[1] + 1], this.lngGrid_[cell[0] + 1]));
    };
    ...
    initGoogle() {
        ...
        this.google.maps.LatLng.prototype.rhumbDestinationPoint = function (brng, dist) {
        ...
        this.google.maps.LatLng.prototype.rhumbBearingTo = function (dest) {
